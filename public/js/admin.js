...
async function loadPayments() {
  ...
  try {
    ...
    payments.forEach(p => {
      const tr = document.createElement('tr');
      const date = new Date(p.createdAt).toLocaleString('ar-MA', { dateStyle: 'medium', timeStyle: 'short' });
      tr.innerHTML = `
        <td>${p.clinicName}</td>
        <td style="font-weight:700;color:${p.amount > 0 ? 'var(--success)' : 'var(--danger)'};">${p.amount > 0 ? '+' : ''}${p.amount}</td>
        <td>${p.note}</td>
        <td style="font-size:0.8rem;color:var(--ink-muted);">${date}</td>
      `;
