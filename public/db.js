...
function updateClinic(id, updates) {
  ...
  db.prepare(`
    UPDATE clinics SET name=?, phone=?, workStart=?, workEnd=?,
      ...
      paymentNumber=?, paymentInstructions=?, requirePaymentProof=?
    WHERE id=?
  `).run(
    ...
    merged.paymentNumber, merged.paymentInstructions, merged.requirePaymentProof, id
  );
  return findClinicById(id);
}
...
module.exports = {
  ...
  findOrCreatePatient, findPatientById, getPatientHistory,
