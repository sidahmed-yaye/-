...
function createClinic(clinic) {
  ...
  db.prepare(`
    INSERT INTO clinics (id, name, phone, username, passwordHash, slug, workStart, workEnd, slotMinutes, status, trialEndsAt, specialty, city, createdAt)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'active', ?, ?, ?, ?)
  `).run(
    id,
    clinic.name,
    ...
    clinic.passwordHash,
    clinic.slug,
    clinic.workStart || '08:00',
    clinic.workEnd || '17:00',
...
function updateClinic(id, updates) {
  ...
  const allowed = ['name', 'phone', 'workStart', 'workEnd', 'slotMinutes', 'status', 'specialty', 'city', 'paymentServiceName', 'paymentNumber', 'paymentInstructions'];
  const merged = {};
  ...
  db.prepare(`
    UPDATE clinics SET name=?, phone=?, workStart=?, workEnd=?,
      ...
      paymentNumber=?, paymentInstructions=?
    WHERE id=?
  `).run(
    ...
    merged.paymentNumber, merged.paymentInstructions, id
  );
  return findClinicById(id);
}
...
function createAppointment(appt) {
  ...
  try {
    db.prepare(`
      INSERT INTO appointments (id, clinicId, patientId, time, status, reason, notes, source, reminderSent, confirmedAt, statusUpdatedAt, createdAt)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, 0, ?, ?, ?)
    `).run(
      id,
      appt.clinicId,
...
function updateAppointment(id, clinicId, updates) {
  ...
  for (const key of allowed) {
    if (updates[key] !== undefined) {
      merged[key] = key === 'reminderSent' ? (updates[key] ? 1 : 0) : updates[key];
    } else {
  ...
  try {
    db.prepare(`
      UPDATE appointments SET status=?, time=?, notes=?, reason=?,
        reminderSent=?, reminderSentAt=?, confirmedAt=?, statusUpdatedAt=?
      WHERE id=? AND clinicId=?
    `).run(
      merged.status, merged.time, merged.notes, merged.reason,
      merged.reminderSent, merged.reminderSentAt, merged.confirmedAt, merged.statusUpdatedAt,
      id, clinicId
    );
  } catch (e) {
    if (e.code === 'SQLITE_CONSTRAINT_UNIQUE' || /UNIQUE/.test(e.message)) {
