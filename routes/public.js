...
router.get('/:slug', (req, res) => {
  ...
  const withHours = db.parseWeeklyHours(clinic);
  res.json({
    name: clinic.name,
    slug: clinic.slug,
    phone: clinic.phone,
    ...
    weeklyHours: withHours.weeklyHours,
...
// توليد الأوقات المتاحة حسب برنامج اليوم المحدد من الأسبوع (كل يوم له دوامه الخاص)
// نحسب يوم الأسبوع بأمان بافتراض توقيت UTC+0 (موريتانيا) بدون الاعتماد على منطقة خادم الاستضافة
function getDaySchedule(clinic, date) {
  const dayIndex = new Date(date + 'T00:00:00Z').getUTCDay();
  const dayKey = DAY_KEYS[dayIndex];
  const clinicWithHours = db.parseWeeklyHours(clinic);
  return clinicWithHours.weeklyHours[dayKey] || { open: false };
}

function generateSlots(clinic, date) {
  const daySchedule = getDaySchedule(clinic, date);
...
// عرض ملف طبي شارَكته العيادة مباشرة مع المريض (عبر رابط واتساب، بدون تسجيل دخول)
router.get('/documents/view/:token', (req, res) => {
  const doc = db.findDocumentByShareToken(req.params.token);
  if (!doc || doc.sharingMode !== 'patient') {
    return res.status(404).json({ error: 'الملف غير موجود أو الرابط غير صالح' });
  }
